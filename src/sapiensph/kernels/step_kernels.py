...
def kinematic_update(
    ...
    x_i_new = wp.min(wp.max(x_i_new, wp.vec3(-0.01, -0.01, 0.0)), wp.vec3(0.11, 0.20, 1e9))
...
def compute_densities(
    ...
    # err_max = 1.0 * rest_density
    err_max = 1e9 * rest_density
    err_min = -0.0001 * rest_density
    rho_err_i = wp.clamp(rho_err_i, err_min, err_max)
